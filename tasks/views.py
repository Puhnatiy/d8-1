...
def cached_page(request):
    #return HttpResponse(f"<p>Time: {datetime.now().isoformat()}</p>")
    return HttpResponse(f"<p>Date: {datetime.now().strftime('%A, %d  %B %Y Time: %H:%M:%S')}.</p><p>Wait 5 minutes to update.</p>")
